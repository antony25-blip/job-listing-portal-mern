          ...
          if (data.success) {
            ...
            try {
              if (data.role === 'jobseeker') {
                const profileRes = await apiRequest("GET", "/api/profile/jobseeker");
                if (profileRes.ok) {
                  const profileData = await profileRes.json();
                  if (profileData.success && profileData.profile) {
                    userData = {
                      ...userData,
                      name: profileData.profile.fullName || userData.name,
                ...
                const profileRes = await apiRequest("GET", "/api/profile/employer");
                if (profileRes.ok) {
                  const profileData = await profileRes.json();
                  if (profileData.success && profileData.profile) {
                    userData = {
                      ...userData,
                      company: profileData.profile.companyName,
                      location: profileData.profile.location,
                      phone: profileData.profile.phone,
              ...
              console.error("Failed to fetch detailed profile:", profileErr);
    ...
    try {
      ...
      if (data.success) {
        ...
        try {
          ...
          console.error("Failed to fetch detailed profile on login:", profileErr);
        }
